...
export function useAuthContainer(networkContainer: Container<NetworkContainer>) {
    return (): AuthContainer => {
        ...
        const setNotificationDeviceId = (token: string) =>
            updateUserApi(authenticatedUser?.id as number, token)
                .then(user => setAuthenticatedUser(user))

        const resetNotificationDeviceId = () => updateUserApi(authenticatedUser?.id as number, null)

        const logout = async () => {
            await resetNotificationDeviceId()
...
function updateUserApi(userId: number, token: string | null): Promise<UserResource> {
